...
void imprime_nos_visitados(Capital capital) {
    printf("%s, ", capital.nome);
}

void imprime_rota(int origem, int destino, int* predecessores, Capital capitais[]) {
    if (destino == origem) {
        printf("%s", capitais[origem].nome);
    } else if (predecessores[destino] == -1) {
        printf("Não existe caminho de %s até %s.\n", capitais[origem].nome, capitais[destino].nome);
    } else {
        imprime_rota(origem, predecessores[destino], predecessores, capitais);
        printf(" -> %s", capitais[destino].nome);
    }
}
...
int CentroMaisProximoBFS(Grafo* grafo, int verticeInicial, Capital capitais[]) { 
    ...
    int nivelDaArvore = 1;
    int countNumVisitados = 0;
    int predecessores[NUM_CAPITAIS];
    for (int i = 0; i <= 26; i++) predecessores[i] = -1;
    ...
    while (!estaVazia(fila)) {
        ...
        if (capitais[vertice].eCentroDistribuicao) {
            printf("Caminho: "); imprime_rota(verticeInicial, vertice, predecessores, capitais);
            printf(" -> %s", capitais[vertice].nome);    
            printf("\n - Centro de distribuição mais próximo de %s é %s, com %d pulos.\n", capitais[verticeInicial].nome, capitais[vertice].nome, distancia[vertice]);
            return vertice; 
        }

        Nodo* temp = grafo->lista[vertice].cabeca;
        printf("Nós visitados do nível da árvore (%d): ", nivelDaArvore);
        while (temp) {
            int adjVertice = temp->vertice;
            countNumVisitados++;
            imprime_nos_visitados(capitais[adjVertice]);
            if (!visitados[adjVertice]) {
                visitados[adjVertice] = 1;
                distancia[adjVertice] = distancia[vertice] + 1;
                predecessores[adjVertice] = vertice;
                enfileirar(fila, adjVertice);
            }
            
            if (capitais[temp->vertice].eCentroDistribuicao) {
                printf("\nTotal de nós visitados: %d\n", countNumVisitados);
                printf("Caminho: "); imprime_rota(verticeInicial, vertice, predecessores, capitais);
                printf(" -> %s", capitais[temp->vertice].nome);    
                printf("\n - Centro de distribuição mais próximo de %s é %s, com %d pulos.\n", capitais[verticeInicial].nome, capitais[temp->vertice].nome, distancia[temp->vertice]);
                return temp->vertice;
            }
            temp = temp->prox;
...
int CentroMaisProximoDFS(Grafo* grafo, int verticeInicial, Capital capitais[]) { 
    ...
    while (!estaVaziaPilha(pilha)) {
        ...
        Nodo* temp = grafo->lista[vertice].cabeca;
        printf("Nós visitados do nível (%d): ", nivelDaArvore);
        while (temp) {
            int adjVertice = temp->vertice;
            countNumVisitados++;
            imprime_nos_visitados(capitais[adjVertice]);
        ...
        if (capitais[vertice].eCentroDistribuicao) {
            if ((tipoOrg == 'M' || tipoOrg == 'K') && !FilaVaziaOrgaos(filas[vertice])) {
                if (TemOTipoFila(filas[vertice], tipoOrg)) {
                    printf("Caminho: "); imprime_rota(verticeInicial, vertice, predecessores, capitais);
                    printf(" -> %s", capitais[vertice].nome);    
                    printf("\n - Centro de distribuição mais próximo de %s, com o órgão do tipo '%c', é %s, com %d pulos.\n", capitais[verticeInicial].nome, tipoOrg, capitais[vertice].nome, distancia[vertice]);
                    return vertice;
                }
            }
            if (tipoOrg == 'C' && !PilhaVaziaOrgaos(pilhas[vertice])) {
